...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        word = word_embedding_forward(captions_in, W_embed)

        if self.cell_type == 'rnn':
            h = rnn_forward(word, h0, Wx, Wh, b)

        elif self.cell_type == 'lstm':
            h = lstm_forward(word, h0, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        word = torch.tensor([self._start] * N).to(features.device)
        ...
        for t in range(max_length):
            ...
            if self.cell_type == 'rnn':
                ...
                prev_h = rnn_step_forward(embed, prev_h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                prev_h, prev_c = lstm_step_forward(embed, prev_h, prev_c, Wx, Wh, b)
                
            scores = prev_h @ W_vocab + b_vocab
            
            best_word = torch.argmax(scores, dim=1)
            
            captions[:, t] = best_word
